...
<head><meta charset="UTF-8"><title>Booking Requests - Owner</title>
...
<div class="main-content">
    <div class="page-header"><h1>Booking Requests</h1></div>
    <c:if test="${not empty param.msg}"><div class="alert alert-success">✅ ${param.msg}</div></c:if>
    <div class="table-card">
        <table>
            <thead><tr><th>#</th><th>Customer</th><th>Hall</th><th>Event Date</th><th>Type</th><th>Guests</th><th>Amount</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
            <c:forEach var="b" items="${bookings}">
            <tr>
                <td>#${b.bookingId}</td>
                <td>${b.customerName}<br><small style="color:var(--gray)">${b.customerPhone}</small></td>
                <td>${b.hallName}</td>
            ...
            <c:if test="${empty bookings}">
                <tr><td colspan="9" style="text-align:center;padding:30px;color:var(--gray);">No booking requests yet.</td></tr>
            </c:if>
            </tbody>
        </table>
    </div>

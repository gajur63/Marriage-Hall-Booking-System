...
<%
    ...
    for (Object o : myBookings) {
        com.javaproject.marriagehallbookingsystem.model.Booking b = (com.javaproject.marriagehallbookingsystem.model.Booking) o;
...
<div class="main-content">
    <div class="welcome-banner" style="background:linear-gradient(135deg,#6a1238,#8B1A4A);border-radius:14px;padding:26px 30px;color:white;margin-bottom:24px;display:flex;justify-content:space-between;align-items:center;">
        <div><h2 style="font-size:1.4rem;font-weight:800;margin-bottom:4px;">Welcome, ${user.fullName}! <i class="fa-solid fa-house" style="color: rgb(0, 0, 0);"></i>
        </h2>
        <p style="opacity:0.85;font-size:0.9rem;">Manage your halls and approve booking requests from here.</p></div>
        <span style="font-size:3.5rem;opacity:0.3;"><i class="fa-solid fa-house" style="color: rgb(0, 0, 0);"></i>
</span>
    </div>

    <div class="stats-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:26px;">
        <div class="stat-card"><div class="stat-icon icon-primary"><i class="fa-solid fa-house"></i></div><div class="stat-info"><div class="stat-value">${hallCount}</div><div class="stat-label">My Halls</div></div></div>
        <div class="stat-card"><div class="stat-icon icon-secondary"><i class="fa-solid fa-file-lines"></i></div><div class="stat-info"><div class="stat-value">${bookingCount}</div><div class="stat-label">Total Bookings</div></div></div>
        <div class="stat-card"><div class="stat-icon icon-danger"><i class="fa-solid fa-hourglass-half"></i>
        </div><div class="stat-info"><div class="stat-value">${pendingCount}</div><div class="stat-label">Pending</div></div></div>
        <div class="stat-card"><div class="stat-icon icon-success"><i class="fa-solid fa-check"></i></div><div class="stat-info"><div class="stat-value">${approvedCount}</div><div class="stat-label">Approved</div></div></div>
    </div>
    ...
    <div style="display:flex;gap:10px;margin-bottom:24px;flex-wrap:wrap;">
        ...
        <a href="${pageContext.request.contextPath}/BookingServlet?action=ownerBookings" class="btn btn-outline"><i class="fa-solid fa-file-lines"></i> View All Bookings</a>
    </div>
    ...
    <div class="table-card" style="margin-bottom:24px;">
        ...
        <div style="padding:14px 18px;border-bottom:1px solid var(--border);display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;">
            <div>
                ...
                <div style="font-size:0.82rem;color:var(--gray);"><i class="fa-solid fa-calendar-days"></i> <fmt:formatDate value="${b.eventDate}" pattern="dd MMM yyyy"/> &nbsp;·&nbsp; ${b.eventType} &nbsp;·&nbsp; <i class="fa-solid fa-people-group" style="color: rgb(0, 0, 0);"></i>
                ${b.guestCount} guests</div>
            </div>
            <div style="display:flex;gap:8px;">
                <form method="post" action="${pageContext.request.contextPath}/BookingServlet" style="display:inline;">
                    ...
                    <button class="btn btn-success btn-sm"><i class="fa-solid fa-circle-check"></i> Approve</button>
                </form>
                <form method="post" action="${pageContext.request.contextPath}/BookingServlet" style="display:inline;">
                    <input type="hidden" name="action" value="reject"><input type="hidden" name="bookingId" value="${b.bookingId}">
                    <button class="btn btn-danger btn-sm"><i class="fa-solid fa-circle-xmark"></i> Reject</button>
                </form>
            </div>
        </div>
        </c:if>
        ...
        <c:if test="${!hasPending}">
            <div class="empty-state"><div class="empty-icon"><i class="fa-solid fa-circle-check"></i></div><p>No pending requests.</p></div>
        </c:if>
    </div>

    <!-- My Halls summary -->
    <div class="table-card">
        <div class="table-header"><h3>=<i class="fa-solid fa-house" style="color: rgb(0, 0, 0);"></i>
            My Halls</h3>
